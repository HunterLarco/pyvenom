...
class IndexYamlFromFile(IndexYaml):
  ...
  def __init__(self, yaml):
    manual, automatic = self._slice_yaml(yaml)
    self.automatic = automatic
    super(IndexYamlFromFile, self).__init__(manual)
  
  def _slice_yaml(self, yaml):
    marker_index = yaml.find(self.marker)
    if marker_index == -1:
      return yaml, None
    
    manual = yaml[:marker_index]
    automatic = yaml[marker_index + len(self.marker):]
    return manual, automatic
  
  def __str__(self):
    if self.automatic:
      super_str = super(IndexYamlFromFile, self).__str__()
      return '{}\n\n{}{}'.format(super_str, self.marker, self.automatic)
    return super(IndexYamlFromFile, self).__str__()
